...
class TTFSCrossEntropyLoss(Layer):
    ...
    def _find_first_spikes(self):
        ...
        for neuron in range(self.parameters.n):
            sorted_spikes = sorted(filter(lambda x: x.source_neuron == neuron, self.input_spikes), key=lambda x: x.time)
    ...
    def get_loss(self, correct_label_neuron : int):
        ...
        loss = 0
        sum0 = sum([np.exp(-spike.time/self.parameters.tau0) for spike in self.first_spikes if spike is not None])
    ...
    def get_classification_result(self, correct_label_neuron : int):
        ...
        if all([x.time > label_time for x in self.first_spikes if x is not label_spike and x is not None]):
            return 1
        else:
            return 0
    ...
    def backward(self, correct_label_neuron : int):
        ...
        tau0, tau1, alpha = self.parameters.tau0, self.parameters.tau1, self.parameters.alpha
        sum0 = sum([np.exp(-spike.time/tau0) for spike in self.first_spikes if spike is not None])
        # compute error for label neuron first
        label_error = 0
        t_label = self.first_spikes[correct_label_neuron].time
        exp_t_label = np.exp(-t_label/tau0)
        exp_t_label_squared = np.exp(-2*t_label/tau0)
        label_error += - (sum0/exp_t_label) * (-exp_t_label/(tau0*sum0) + exp_t_label_squared/(tau0*sum0**2))
