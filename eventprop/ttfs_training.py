...
class TwoLayerTTFS(ABC):
    def __init__(self, gd_parameters : GradientDescentParameters = GradientDescentParameters(),
    ...
    weight_increase_threshold_hidden : float = .3,
    weight_increase_threshold_output : float = .0,
    ...
    def save_to_file(self, fname):
        pickle.dump((self.losses, self.accuracies, self.test_accuracies, self.test_losses, self.valid_accuracies, self.valid_losses, self.weights), open(fname, "wb"))

    def _get_minibatch(self):
        if self.gd_parameters.batch_size is None:
            return self.train_spikes
        else:
            samples = deepcopy(self.train_spikes[self._minibatch_idx:self._minibatch_idx+self.gd_parameters.batch_size])
    ...
    def train(self, save_to : str = None, save_every : int = 100, test_every : int = 100, valid_every : int = 100):
        ...
        for epoch in range(self.gd_parameters.epochs):
            ...
            for pattern in minibatch:
                ...
                batch_classif_results.append(self.loss.get_classification_result(pattern.label))
                frac_quiet_output.append(sum([len(x) == 0 for x in self.output_layer._post_spikes_per_neuron])/self.output_layer.parameters.n)
                frac_quiet_hidden.append(sum([len(x) == 0 for x in self.hidden_layer._post_spikes_per_neuron])/self.hidden_layer.parameters.n)
            frac_quiet_output, frac_quiet_hidden = np.mean(frac_quiet_output), np.mean(frac_quiet_hidden)
            logging.debug(f"Training loss in epoch {epoch}: {np.nanmean(batch_losses)}")
            logging.debug(f"Training accuracy in epoch {epoch}: {np.mean(batch_classif_results)}")
            ...
            if save_to is not None:
                if epoch % save_every == 0:
                    self.weights.append((self.hidden_layer.w_in.copy(), self.output_layer.w_in.copy()))
