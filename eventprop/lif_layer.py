...
class LIFLayer(Layer):
    ...
    def _v(self, t: float, target_nrn_idx: int) -> float:
        ...
        post_times = np.array(
            [
                s.time
                for s in self._post_spikes_per_neuron[target_nrn_idx]
                if s.time <= t
            ]
        )
        v += np.sum(
            -self.parameters.v_th * np.exp(-(t - post_times) / self.parameters.tau_mem)
        )
        return v
    ...
    def _bracket_spike(self, t_before, t_after, target_nrn_idx):
        return brentq(
            lambda t: self._v(t, target_nrn_idx) - 1, t_before, t_after, disp=True
        )
    ...
    def backward(self, output_spikes: List[Spike] = None, code: str = "cpp"):
        ...
        if code == "cpp":
            ...
            for target_nrn_idx in range(self.parameters.n):
                ...
                nrn_events = (
                    self.input_spikes + self._post_spikes_per_neuron[target_nrn_idx]
                )
                if len(nrn_events) == 0:
                    continue
                ...
                for event in nrn_events:
                    ...
                    if is_pre_spike(event):
                        ...
                        self.gradient[event.source_neuron, target_nrn_idx] += (
                            -self.parameters.tau_syn * lambda_i
                        )
                        outbound_signal = self.w_in[
                            event.source_neuron, target_nrn_idx
                        ] * (lambda_v - lambda_i)
                    ...
                    # post spike -> jump and absorb error
                        ...
                        lambda_i_jump = (
                            ...
                            * (self.parameters.v_th * lambda_v + event.error)
                        )
                        lambda_i_spikes.append((lambda_i_jump, t_bwd))
                        # self-jump and error absorption
                        lambda_v = (
                            i / (i - self.parameters.v_th) * lambda_v
                            + 1 / (i - self.parameters.v_th) * event.error
                        )
                    previous_t = t_bwd
        self._ran_backward = True
        super().backward()
            ...
            for i, spike in enumerate(self.input_spikes + [final_spike]):
                ...
                if x is not None:
                    ...
                    if vmax > self.parameters.v_th + np.finfo(float).eps:
                        ...
                        post_spike = Spike(
                            time=t0, source_neuron=target_nrn_idx, source_layer=id(self)
                        )
                        ...
                        break
                if (
                    self._v(spike.time, target_nrn_idx)
    ...
    def _i(self, t: float, target_nrn_idx: int) -> float:
        ...
        for spike in self.input_spikes:
            if spike.time <= t:
                i += self.w_in[spike.source_neuron, target_nrn_idx] * np.exp(
                    -(t - spike.time) / self.parameters.tau_syn
                )
        return i
