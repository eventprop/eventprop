...
w_hidden = np.random.normal(0.005, 0.05, size=(hidden_parameters.n_in, hidden_parameters.n))
w_output = np.random.normal(5.0, 0.0, size=(output_parameters.n_in, output_parameters.n))
...
ts, output_voltage_trace = output_layer.get_voltage_trace_for_neuron(0, plot_t_max, dt=1e-4)
ts, hidden_voltage_trace = hidden_layer.get_voltage_trace_for_neuron(0, plot_t_max, dt=1e-4)

ts, output_lambda_trace = output_layer.get_lambda_i_trace_for_neuron(0, plot_t_max, dt=1e-4)
output_lambdas = [output_layer.get_lambda_i_for_neuron(0, x.time) for x in hidden_spikes]
ts, hidden_lambda_trace = hidden_layer.get_lambda_i_trace_for_neuron(0, plot_t_max, dt=1e-4)
hidden_lambdas = [hidden_layer.get_lambda_i_for_neuron(0, x.time) for x in input_spikes if x.source_neuron == 0]

def plot(ts, pre_spike_times, voltage_trace, lambda_trace, lambdas, tau_syn, gradient, numerical_gradient, save_to):
